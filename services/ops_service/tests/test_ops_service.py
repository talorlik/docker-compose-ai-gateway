"""Unit tests for ops service /handle and /health endpoints."""
...
def test_handle_returns_200(client):
    resp = client.post(
        "/handle",
        json={"request_id": "test-001", "text": "kubectl get pods crashloopbackoff"},
    )
    assert resp.status_code == 200
...
def test_handle_response_shape(client):
    resp = client.post(
        "/handle",
        json={"request_id": "test-002", "text": "check node readiness"},
    )
    data = resp.json()
    ...
    assert payload["service"] == "ops-service"
    assert "result" in payload
    assert "instance" in payload
    assert "suggested_commands" in payload
    assert "status_checks" in payload

    trace = data["trace_append"]
    assert trace["service"] == "ops-service"
